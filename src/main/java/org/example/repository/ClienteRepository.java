...
public class ClienteRepository extends _BaseRepositoryImpl<Cliente> {
    ...
    @Query("INSERT INTO GS_CLIENTE (NOME, EMAIL, TELEFONE, GS_ENDERECO_COD_ENDERECO) VALUES (?, ?, ?, ?)")
    public void create(Cliente cliente, int enderecoId) {
        try {
            QueryProcessor.executeAnnotatedMethod(this, "create", cliente.getNome(), cliente.getEmail(),
                    cliente.getTelefone(), enderecoId);
